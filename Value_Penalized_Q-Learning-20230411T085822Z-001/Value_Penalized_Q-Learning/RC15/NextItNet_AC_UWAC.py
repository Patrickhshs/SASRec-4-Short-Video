...
def parse_args():
    parser = argparse.ArgumentParser(description="Next-AC-UWAC.")
...
class NextItNet:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, coef, num_multi_head,
        ...
        with tf.variable_scope(self.name):
            ...
            # self.targetQ_unc = tf.placeholder(tf.float32, [None, item_num])
            self.reward = tf.placeholder(tf.float32, [None])
            self.discount = tf.placeholder(tf.float32, [None])

            std = tf.math.reduce_std(multi_head_output, axis=-1)
            std = indexing_ops.batched_index(std, self.actions)
            unc_weights = tf.stop_gradient(coef / (std + 0.001))
            ...
            naive_celoss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, 
                                                                            logits=self.output2)
            q_indexed = tf.stop_gradient(tf.multiply(unc_weights, q_indexed))
...
def evaluate(sess, i):
    ...
    while evaluated<len(eval_ids):
        ...
        prediction, state_hidden=sess.run([NextRec1.output2, NextRec1.state_hidden],
            feed_dict={NextRec1.inputs: states,NextRec1.len_state:len_states,NextRec1.is_training:False})
        if args.logging is True and i % 5 ==0:
            if np.random.randint(50) == 1:
                print('logging', end ='...')
            np.savetxt('state_distribution_shift/test_state_hidden_epoch_' +str(i)+ '_batch_'+ str(evaluated) + '.csv', 
                        np.array(state_hidden), delimiter=',')
        n_jobs=5
...
if __name__ == '__main__':
        ...
        for i in range(args.epoch):
            for j in range(num_batches):
                ...
                if pointer == 0:
                    ...
                    mainQN = NextRec2
                    target_QN = NextRec1
                # target_Qs target_Qs_selector, 1 x 40783 
                ...
                loss, _, qloss, celoss, naive_celoss, q_mean, q_std, q_max, state_hidden = sess.run([
                ...
                if args.logging is True and i % 5 ==0 :
                    np.savetxt('state_distribution_shift/train_state_hidden_epoch_' + str(i) + 'batch_'+ str(j) + '.csv', 
                                np.array(state_hidden), delimiter=',')
                if total_step % 50000 == 0:
                    if total_step < 2000 * 15:
                        pass
                    else:
                        print('\nstart to eval')
                        time_eval_start = time.time()
                        log_data_one_eval = evaluate(sess, i)
    ...
    print('time used in NextItNet_AC_UWAC :', time.time() - start_time)
    log_data = pd.DataFrame(log_data, columns=column_name)
    log_data.to_csv('log_data_rem/' + args.out + '.csv')
    pd.DataFrame(q_mean_log).to_csv('log_data_rem/q_mean_log_' + args.out + '.csv')
    pd.DataFrame(q_std_log).to_csv('log_data_rem/q_std_log_' + args.out + '.csv')
